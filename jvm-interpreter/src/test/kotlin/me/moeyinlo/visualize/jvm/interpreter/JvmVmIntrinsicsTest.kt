...
class JvmVmIntrinsicsTest {
    ...
    @Test
    fun `Class isAssignableFrom intrinsic rejects reference array covariance across component defining loaders`() {
        val sourceLoader = JvmClassLoaderIdentity.UserDefined(id = 188, displayName = "source")
        val targetLoader = JvmClassLoaderIdentity.UserDefined(id = 189, displayName = "target")
        val sourceArrayKey = JvmLoadedClassKey("[Lpkg/Child;", sourceLoader)
        val targetArrayKey = JvmLoadedClassKey("[Lpkg/Base;", targetLoader)
        val heap = JvmHeap()
        val sourceMirror = heap.internClassMirror("[Lpkg/Child;", loadedClassKey = sourceArrayKey)
    ...
    @Test
    fun `Class isAssignableFrom intrinsic accepts reference array covariance within component defining loader`() {
        val loader = JvmClassLoaderIdentity.UserDefined(id = 190, displayName = "shared")
    ...
    @Test
    fun `Class isAssignableFrom intrinsic follows loaded superclass definitions`() {
        val loader = JvmClassLoaderIdentity.UserDefined(id = 185, displayName = "app")
        ...
        val heap = JvmHeap()
        val baseMirror = heap.internClassMirror("pkg/Base", loadedClassKey = baseKey)
        ...
        val context = JvmNativeMethodContext(
            ...
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(internalName = "pkg/OtherBase"),
                    JvmClassDefinition(internalName = "pkg/Child", superclassName = "pkg/OtherBase"),
                ),
            ),
            staticFields = JvmStaticFields(),
            currentClassName = "java/lang/Class",
    ...
    @Test
    fun `Class isAssignableFrom intrinsic follows loaded direct interface definitions`() {
        val loader = JvmClassLoaderIdentity.UserDefined(id = 186, displayName = "app")
        val serviceKey = JvmLoadedClassKey("pkg/Service", loader)
        val childKey = JvmLoadedClassKey("pkg/Child", loader)
        val methodArea = JvmMethodArea()
        methodArea.defineClass(
            JvmMethodAreaEntry(
                definition = JvmClassDefinition(internalName = "pkg/Service", isInterface = true),
                loadedClassKey = serviceKey,
                initiatingLoaders = setOf(loader),
            ),
        )
        methodArea.defineClass(
        ...
        val heap = JvmHeap()
        val serviceMirror = heap.internClassMirror("pkg/Service", loadedClassKey = serviceKey)
        val childMirror = heap.internClassMirror("pkg/Child", loadedClassKey = childKey)
        val intrinsic = JvmVmIntrinsics.Registry.resolve(classIsAssignableFromMethod())
            ?: error("Class.isAssignableFrom intrinsic was not registered")
        val context = JvmNativeMethodContext(
            ...
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(internalName = "pkg/OtherService", isInterface = true),
                    JvmClassDefinition(internalName = "pkg/Child", interfaceNames = listOf("pkg/OtherService")),
                ),
            ),
            staticFields = JvmStaticFields(),
            currentClassName = "java/lang/Class",
        ...
        assertEquals(
            JvmIntValue(0),
            intrinsic.invoke(context, JvmNativeMethodInvocation(childMirror, listOf(serviceMirror))),
        )
    }
    ...
    @Test
    fun `Class getNestHost0 intrinsic returns loader qualified nest host mirrors`() {
        val loader = JvmClassLoaderIdentity.UserDefined(id = 212, displayName = "app")
        val hostKey = JvmLoadedClassKey("pkg/Host", loader)
        val nestedKey = JvmLoadedClassKey("pkg/Host\$Nested", loader)
        val methodArea = JvmMethodArea()
        methodArea.defineClass(
            JvmMethodAreaEntry(
                definition = JvmClassDefinition(internalName = "pkg/Host"),
        ...
        val intrinsic = JvmVmIntrinsics.Registry.resolve(classGetNestHost0Method())
            ?: error("Class.getNestHost0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(emptyList()),
            staticFields = JvmStaticFields(),
        ...
        val selfHost = intrinsic.invoke(context, JvmNativeMethodInvocation(hostMirror, emptyList()))
            as JvmObjectReferenceValue
        val nestedHost = intrinsic.invoke(context, JvmNativeMethodInvocation(nestedMirror, emptyList()))
            as JvmObjectReferenceValue

        assertEquals(JvmClassPayload("pkg/Host", loadedClassKey = hostKey), heap.get(selfHost).payload)
        assertEquals(JvmClassPayload("pkg/Host", loadedClassKey = hostKey), heap.get(nestedHost).payload)
    }
    ...
    fun `Class getComponentType intrinsic returns array component mirrors`() {
        ...
        val nestedArrayKey = JvmLoadedClassKey("[[Lpkg/Child;", loader)
        val heap = JvmHeap()
        val intArrayMirror = heap.internClassMirror("[I")
        val referenceArrayMirror = heap.internClassMirror("[Lpkg/Child;", loadedClassKey = referenceArrayKey)
        val nestedArrayMirror = heap.internClassMirror("[[Lpkg/Child;", loadedClassKey = nestedArrayKey)
        val ordinaryMirror = heap.internClassMirror("pkg/Child")
        val primitiveMirror = heap.internClassMirror("int")
        val intrinsic = JvmVmIntrinsics.Registry.resolve(classGetComponentTypeMethod())
            ?: error("Class.getComponentType intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(emptyList()),
            staticFields = JvmStaticFields(),
        ...
        assertEquals(
            heap.internClassMirror("int"),
            intrinsic.invoke(context, JvmNativeMethodInvocation(intArrayMirror, emptyList())),
        )
        val referenceComponent = intrinsic.invoke(
            context,
            JvmNativeMethodInvocation(referenceArrayMirror, emptyList()),
        ) as JvmObjectReferenceValue
        assertEquals(
            JvmClassPayload(
                representedClassName = "pkg/Child",
                loadedClassKey = JvmLoadedClassKey("pkg/Child", loader),
            ),
            heap.get(referenceComponent).payload,
        )
        val nestedComponent = intrinsic.invoke(
            context,
            JvmNativeMethodInvocation(nestedArrayMirror, emptyList()),
        ) as JvmObjectReferenceValue
        assertEquals(
            JvmClassPayload(
                representedClassName = "[Lpkg/Child;",
                loadedClassKey = referenceArrayKey,
            ),
            heap.get(nestedComponent).payload,
        )
        assertEquals(
            JvmNullValue,
    ...
    @Test
    fun `Class getModifiers intrinsic returns class interface array and primitive modifiers`() {
        val loader = JvmClassLoaderIdentity.UserDefined(id = 209, displayName = "app")
        ...
        val intrinsic = JvmVmIntrinsics.Registry.resolve(classGetModifiersMethod())
            ?: error("Class.getModifiers intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(emptyList()),
            staticFields = JvmStaticFields(),
    ...
    fun `Class getClassFileVersion0 intrinsic returns loaded and default major versions`() {
        ...
        val heap = JvmHeap()
        val loadedMirror = heap.internClassMirror("pkg/Versioned", loadedClassKey = loadedKey)
        val hierarchyMirror = heap.internClassMirror("pkg/HierarchyOnly")
        val primitiveMirror = heap.internClassMirror("int")
        val arrayMirror = heap.internClassMirror("[Lpkg/Versioned;")
        val intrinsic = JvmVmIntrinsics.Registry.resolve(classGetClassFileVersion0Method())
            ?: error("Class.getClassFileVersion0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
    ...
    @Test
    fun `Class getInterfaces0 intrinsic preserves loaded class key identity`() {
        val loader = JvmClassLoaderIdentity.UserDefined(id = 179, displayName = "app")
        val childKey = JvmLoadedClassKey("pkg/Child", loader)
        val serviceKey = JvmLoadedClassKey("pkg/Service", loader)
        val markerKey = JvmLoadedClassKey("pkg/Marker", loader)
        val methodArea = JvmMethodArea()
        methodArea.defineClass(
            JvmMethodAreaEntry(
                definition = JvmClassDefinition(
        ...
        methodArea.defineClass(
            JvmMethodAreaEntry(
                definition = JvmClassDefinition(internalName = "pkg/Marker", isInterface = true),
                loadedClassKey = markerKey,
                initiatingLoaders = setOf(loader),
            ),
        )
        val heap = JvmHeap()
        ...
        assertEquals(
            JvmClassPayload("pkg/Service", loadedClassKey = serviceKey),
            heap.get(payload.elements[0] as JvmObjectReferenceValue).payload,
        )
        assertEquals(
            JvmClassPayload("pkg/Marker", loadedClassKey = markerKey),
    ...
    @Test
    fun `Unsafe reallocateMemory0 intrinsic moves grown blocks instead of overlapping later allocations`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val first = memory.allocateNativeMemory(8L)
        val second = memory.allocateNativeMemory(8L)
        memory.setNativeMemory(first, 8L, 0x11.toByte())
        memory.setNativeMemory(second, 8L, 0x22.toByte())
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeReallocateMemory0Method())
            ?: error("Unsafe.reallocateMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val result = intrinsic.invoke(context, JvmNativeMethodInvocation(unsafe, listOf(JvmLongValue(first), JvmLongValue(16L))))
            as JvmLongValue

        assertNotEquals(first, result.value)
        assertEquals(null, memory.nativeMemoryBlockSize(first))
        assertEquals(16L, memory.nativeMemoryBlockSize(result.value))
        assertEquals(8L, memory.nativeMemoryBlockSize(second))
        for (offset in 0L until 8L) {
            assertEquals(0x11.toByte(), memory.nativeMemoryByte(result.value + offset))
            assertEquals(0x22.toByte(), memory.nativeMemoryByte(second + offset))
        }
        for (offset in 8L until 16L) {
            assertEquals(0.toByte(), memory.nativeMemoryByte(result.value + offset))
        }
    }
    ...
    @Test
    fun `Unsafe setMemory0 intrinsic fills guest boolean arrays from synthetic base offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val array = heap.allocateBooleanArray(5)
        val payload = heap.get(array).payload as JvmBooleanArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeSetMemory0Method())
            ?: error("Unsafe.setMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(array, JvmLongValue(1L), JvmLongValue(3L), JvmIntValue(1)),
            ),
        )
    ...
    @Test
    fun `Unsafe setMemory0 intrinsic fills guest char arrays from synthetic byte offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val array = heap.allocateCharArray(4)
        val payload = heap.get(array).payload as JvmCharArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeSetMemory0Method())
            ?: error("Unsafe.setMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(array, JvmLongValue(2L), JvmLongValue(4L), JvmIntValue(0x41)),
            ),
        )

        assertEquals(null, result)
        assertEquals(listOf('\u0000', '\u4141', '\u4141', '\u0000'), payload.elements)
    }

    @Test
    fun `Unsafe setMemory0 intrinsic fills guest int arrays from synthetic byte offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val array = heap.allocateIntArray(4)
        val payload = heap.get(array).payload as JvmIntArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeSetMemory0Method())
            ?: error("Unsafe.setMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
    ...
    @Test
    fun `Unsafe setMemory0 intrinsic fills guest long arrays from synthetic byte offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val array = heap.allocateLongArray(4)
        val payload = heap.get(array).payload as JvmLongArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeSetMemory0Method())
            ?: error("Unsafe.setMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        assertEquals(null, result)
        assertEquals(listOf(0L, 0x1212121212121212L, 0x1212121212121212L, 0L), payload.elements)
    }
    ...
    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest byte arrays from synthetic base offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateByteArray(5)
        val target = heap.allocateByteArray(5)
        val sourcePayload = heap.get(source).payload as JvmByteArrayPayload
        val targetPayload = heap.get(target).payload as JvmByteArrayPayload
        sourcePayload.elements[0] = 0x01
        sourcePayload.elements[1] = 0x02
        sourcePayload.elements[2] = 0x03
        sourcePayload.elements[3] = 0x04
        sourcePayload.elements[4] = 0x05
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(1L), target, JvmLongValue(2L), JvmLongValue(3L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(listOf(0.toByte(), 0.toByte(), 0x02.toByte(), 0x03.toByte(), 0x04.toByte()), targetPayload.elements)
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies synthetic native memory into guest byte arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(4L)
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(JvmNullValue, JvmLongValue(source + 1L), target, JvmLongValue(2L), JvmLongValue(2L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(listOf(0.toByte(), 0.toByte(), 0x22.toByte(), 0x22.toByte(), 0.toByte()), targetPayload.elements)
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest byte arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = heap.allocateByteArray(5)
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(1L), JvmNullValue, JvmLongValue(target + 1L), JvmLongValue(3L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(0, memory.nativeMemoryByte(target))
        assertEquals(0x02.toByte(), memory.nativeMemoryByte(target + 1L))
        assertEquals(0x03.toByte(), memory.nativeMemoryByte(target + 2L))
        assertEquals(0x04.toByte(), memory.nativeMemoryByte(target + 3L))
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest boolean arrays from synthetic base offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateBooleanArray(5)
        val target = heap.allocateBooleanArray(5)
        val sourcePayload = heap.get(source).payload as JvmBooleanArrayPayload
        val targetPayload = heap.get(target).payload as JvmBooleanArrayPayload
        sourcePayload.elements[0] = true
        sourcePayload.elements[1] = false
        sourcePayload.elements[2] = true
        sourcePayload.elements[3] = true
        sourcePayload.elements[4] = false
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        assertEquals(null, result)
        assertEquals(listOf(false, false, false, true, true), targetPayload.elements)
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest short arrays from synthetic byte offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateShortArray(4)
        val target = heap.allocateShortArray(4)
        val sourcePayload = heap.get(source).payload as JvmShortArrayPayload
        val targetPayload = heap.get(target).payload as JvmShortArrayPayload
        sourcePayload.elements[0] = 0x1122.toShort()
        sourcePayload.elements[1] = 0x3344.toShort()
        sourcePayload.elements[2] = 0x5566.toShort()
        sourcePayload.elements[3] = 0x7788.toShort()
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(2L), target, JvmLongValue(2L), JvmLongValue(4L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(
            listOf(0.toShort(), 0x3344.toShort(), 0x5566.toShort(), 0.toShort()),
            targetPayload.elements,
        )
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest char arrays from synthetic byte offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateCharArray(4)
        val target = heap.allocateCharArray(4)
        val sourcePayload = heap.get(source).payload as JvmCharArrayPayload
        val targetPayload = heap.get(target).payload as JvmCharArrayPayload
        sourcePayload.elements[0] = '\u1122'
        sourcePayload.elements[1] = '\u3344'
        sourcePayload.elements[2] = '\u5566'
        sourcePayload.elements[3] = '\u7788'
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        assertEquals(null, result)
        assertEquals(listOf('\u0000', '\u3344', '\u5566', '\u0000'), targetPayload.elements)
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest int arrays from synthetic byte offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateIntArray(4)
        val target = heap.allocateIntArray(4)
        ...
        sourcePayload.elements[1] = 0x55667788
        sourcePayload.elements[2] = -0x76543211
        sourcePayload.elements[3] = 0x01020304
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(4L), target, JvmLongValue(4L), JvmLongValue(8L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(listOf(0, 0x55667788, -0x76543211, 0), targetPayload.elements)
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest float arrays from synthetic byte offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateFloatArray(4)
        val target = heap.allocateFloatArray(4)
        val sourcePayload = heap.get(source).payload as JvmFloatArrayPayload
        val targetPayload = heap.get(target).payload as JvmFloatArrayPayload
        sourcePayload.elements[0] = Float.fromBits(0x11223344)
        sourcePayload.elements[1] = Float.fromBits(0x55667788)
        sourcePayload.elements[2] = Float.fromBits(-0x76543211)
        sourcePayload.elements[3] = Float.fromBits(0x01020304)
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        assertEquals(null, result)
        assertEquals(
            listOf(0.0f, Float.fromBits(0x55667788), Float.fromBits(-0x76543211), 0.0f),
            targetPayload.elements,
        )
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest long arrays from synthetic byte offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateLongArray(4)
        val target = heap.allocateLongArray(4)
        val sourcePayload = heap.get(source).payload as JvmLongArrayPayload
        val targetPayload = heap.get(target).payload as JvmLongArrayPayload
        sourcePayload.elements[0] = 0x1122334455667788L
        sourcePayload.elements[1] = 0x0102030405060708L
        sourcePayload.elements[2] = -0x123456789abcdfL
        sourcePayload.elements[3] = 0x7f6e5d4c3b2a1908L
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        assertEquals(null, result)
        assertEquals(listOf(0L, 0x0102030405060708L, -0x123456789abcdfL, 0L), targetPayload.elements)
    }
    ...
    @Test
    fun `Unsafe copyMemory0 intrinsic copies synthetic native memory into guest short arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(4L)
        memory.setNativeMemory(source, 1L, 0x11.toByte())
        memory.setNativeMemory(source + 1L, 1L, 0x11.toByte())
        memory.setNativeMemory(source + 2L, 1L, 0x22.toByte())
        memory.setNativeMemory(source + 3L, 1L, 0x22.toByte())
        val target = heap.allocateShortArray(4)
        val targetPayload = heap.get(target).payload as JvmShortArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(JvmNullValue, JvmLongValue(source), target, JvmLongValue(2L), JvmLongValue(4L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(listOf(0.toShort(), 0x1111.toShort(), 0x2222.toShort(), 0.toShort()), targetPayload.elements)
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies synthetic native memory into guest char arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(4L)
        memory.setNativeMemory(source, 1L, 0x33.toByte())
        memory.setNativeMemory(source + 1L, 1L, 0x33.toByte())
        memory.setNativeMemory(source + 2L, 1L, 0x44.toByte())
        memory.setNativeMemory(source + 3L, 1L, 0x44.toByte())
        val target = heap.allocateCharArray(4)
        val targetPayload = heap.get(target).payload as JvmCharArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        ...
        assertEquals(null, result)
        assertEquals(listOf('\u0000', '\u3333', '\u4444', '\u0000'), targetPayload.elements)
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies synthetic native memory into guest int arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(8L)
        repeat(4) { index -> memory.setNativeMemory(source + index.toLong(), 1L, 0x55.toByte()) }
        repeat(4) { index -> memory.setNativeMemory(source + 4L + index.toLong(), 1L, 0x66.toByte()) }
        val target = heap.allocateIntArray(4)
        val targetPayload = heap.get(target).payload as JvmIntArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(JvmNullValue, JvmLongValue(source), target, JvmLongValue(4L), JvmLongValue(8L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(listOf(0, 0x55555555, 0x66666666, 0), targetPayload.elements)
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies synthetic native memory into guest float arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(8L)
        repeat(4) { index -> memory.setNativeMemory(source + index.toLong(), 1L, 0x21.toByte()) }
        repeat(4) { index -> memory.setNativeMemory(source + 4L + index.toLong(), 1L, 0x42.toByte()) }
        val target = heap.allocateFloatArray(4)
        val targetPayload = heap.get(target).payload as JvmFloatArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        ...
        assertEquals(null, result)
        assertEquals(
            listOf(0.0f, Float.fromBits(0x21212121), Float.fromBits(0x42424242), 0.0f),
            targetPayload.elements,
        )
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies synthetic native memory into guest long arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(16L)
        repeat(8) { index -> memory.setNativeMemory(source + index.toLong(), 1L, 0x12.toByte()) }
        repeat(8) { index -> memory.setNativeMemory(source + 8L + index.toLong(), 1L, 0x34.toByte()) }
        val target = heap.allocateLongArray(4)
        val targetPayload = heap.get(target).payload as JvmLongArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(JvmNullValue, JvmLongValue(source), target, JvmLongValue(8L), JvmLongValue(16L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(listOf(0L, 0x1212121212121212L, 0x3434343434343434L, 0L), targetPayload.elements)
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies synthetic native memory into guest double arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(16L)
        repeat(8) { index -> memory.setNativeMemory(source + index.toLong(), 1L, 0x56.toByte()) }
        repeat(8) { index -> memory.setNativeMemory(source + 8L + index.toLong(), 1L, 0x78.toByte()) }
        val target = heap.allocateDoubleArray(4)
        val targetPayload = heap.get(target).payload as JvmDoubleArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        ...
        assertEquals(null, result)
        assertEquals(
            listOf(0.0, Double.fromBits(0x5656565656565656L), Double.fromBits(0x7878787878787878L), 0.0),
            targetPayload.elements,
        )
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest short arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateShortArray(4)
        val sourcePayload = heap.get(source).payload as JvmShortArrayPayload
        sourcePayload.elements[1] = 0x1111.toShort()
        sourcePayload.elements[2] = 0x2222.toShort()
        val memory = JvmUnsafeSyntheticMemory()
        val target = memory.allocateNativeMemory(4L)
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(2L), JvmNullValue, JvmLongValue(target), JvmLongValue(4L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(
            listOf(0x11.toByte(), 0x11.toByte(), 0x22.toByte(), 0x22.toByte()),
            (0L until 4L).map { offset -> memory.nativeMemoryByte(target + offset) },
        )
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest char arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateCharArray(4)
        val sourcePayload = heap.get(source).payload as JvmCharArrayPayload
        sourcePayload.elements[1] = '\u3333'
        sourcePayload.elements[2] = '\u4444'
        val memory = JvmUnsafeSyntheticMemory()
        val target = memory.allocateNativeMemory(4L)
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        ...
        assertEquals(null, result)
        assertEquals(
            listOf(0x33.toByte(), 0x33.toByte(), 0x44.toByte(), 0x44.toByte()),
            (0L until 4L).map { offset -> memory.nativeMemoryByte(target + offset) },
        )
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest int arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateIntArray(4)
        val sourcePayload = heap.get(source).payload as JvmIntArrayPayload
        sourcePayload.elements[1] = 0x55555555
        sourcePayload.elements[2] = 0x66666666
        val memory = JvmUnsafeSyntheticMemory()
    ...
    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest long arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateLongArray(4)
        val sourcePayload = heap.get(source).payload as JvmLongArrayPayload
        sourcePayload.elements[1] = 0x1212121212121212L
        sourcePayload.elements[2] = 0x3434343434343434L
        val memory = JvmUnsafeSyntheticMemory()
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(8L), JvmNullValue, JvmLongValue(target), JvmLongValue(16L)),
            ),
        )
        ...
        assertEquals(
            listOf(
                0x12.toByte(), 0x12.toByte(), 0x12.toByte(), 0x12.toByte(),
                0x12.toByte(), 0x12.toByte(), 0x12.toByte(), 0x12.toByte(),
                0x34.toByte(), 0x34.toByte(), 0x34.toByte(), 0x34.toByte(),
                0x34.toByte(), 0x34.toByte(), 0x34.toByte(), 0x34.toByte(),
            ),
            (0L until 16L).map { offset -> memory.nativeMemoryByte(target + offset) },
        )
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest double arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateDoubleArray(4)
        val sourcePayload = heap.get(source).payload as JvmDoubleArrayPayload
        sourcePayload.elements[1] = Double.fromBits(0x5656565656565656L)
        sourcePayload.elements[2] = Double.fromBits(0x7878787878787878L)
        val memory = JvmUnsafeSyntheticMemory()
        val target = memory.allocateNativeMemory(16L)
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        ...
        assertEquals(
            listOf(
                0x56.toByte(), 0x56.toByte(), 0x56.toByte(), 0x56.toByte(),
                0x56.toByte(), 0x56.toByte(), 0x56.toByte(), 0x56.toByte(),
                0x78.toByte(), 0x78.toByte(), 0x78.toByte(), 0x78.toByte(),
                0x78.toByte(), 0x78.toByte(), 0x78.toByte(), 0x78.toByte(),
            ),
            (0L until 16L).map { offset -> memory.nativeMemoryByte(target + offset) },
        )
    ...
    fun `Unsafe copyMemory0 intrinsic copies synthetic native memory into guest boolean arrays`() {
        ...
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(4L)
        memory.setNativeMemory(source, 1L, 1.toByte())
        memory.setNativeMemory(source + 1L, 1L, 0.toByte())
        memory.setNativeMemory(source + 2L, 1L, 1.toByte())
        memory.setNativeMemory(source + 3L, 1L, 1.toByte())
        val target = heap.allocateBooleanArray(5)
        val targetPayload = heap.get(target).payload as JvmBooleanArrayPayload
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopyMemory0Method())
            ?: error("Unsafe.copyMemory0 intrinsic was not registered")
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(JvmNullValue, JvmLongValue(source), target, JvmLongValue(1L), JvmLongValue(4L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(listOf(false, true, false, true, true), targetPayload.elements)
    }

    @Test
    fun `Unsafe copyMemory0 intrinsic copies guest boolean arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateBooleanArray(5)
        val sourcePayload = heap.get(source).payload as JvmBooleanArrayPayload
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(1L), JvmNullValue, JvmLongValue(target), JvmLongValue(4L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(0.toByte(), memory.nativeMemoryByte(target))
        assertEquals(1.toByte(), memory.nativeMemoryByte(target + 1L))
        assertEquals(1.toByte(), memory.nativeMemoryByte(target + 2L))
        assertEquals(0.toByte(), memory.nativeMemoryByte(target + 3L))
    }
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest byte array element bytes`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateByteArray(4)
        val target = heap.allocateByteArray(4)
        val sourcePayload = heap.get(source).payload as JvmByteArrayPayload
        val targetPayload = heap.get(target).payload as JvmByteArrayPayload
        sourcePayload.elements[0] = 0x01
        sourcePayload.elements[1] = 0x02
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(0L), target, JvmLongValue(0L), JvmLongValue(4L), JvmLongValue(2L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(listOf(0x02.toByte(), 0x01.toByte(), 0x04.toByte(), 0x03.toByte()), targetPayload.elements)
    }

    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest short array element bytes`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateShortArray(2)
        val target = heap.allocateShortArray(2)
        val sourcePayload = heap.get(source).payload as JvmShortArrayPayload
        val targetPayload = heap.get(target).payload as JvmShortArrayPayload
        sourcePayload.elements[0] = 0x1122.toShort()
        sourcePayload.elements[1] = 0x3344.toShort()
        ...
        assertEquals(null, result)
        assertEquals(listOf(0x2211.toShort(), 0x4433.toShort()), targetPayload.elements)
    }

    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest char array element bytes`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateCharArray(2)
        val target = heap.allocateCharArray(2)
        val sourcePayload = heap.get(source).payload as JvmCharArrayPayload
        val targetPayload = heap.get(target).payload as JvmCharArrayPayload
        sourcePayload.elements[0] = '\u1122'
        sourcePayload.elements[1] = '\u3344'
        ...
        assertEquals(null, result)
        assertEquals(listOf('\u2211', '\u4433'), targetPayload.elements)
    }
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest float array element bytes`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateFloatArray(2)
        val target = heap.allocateFloatArray(2)
        val sourcePayload = heap.get(source).payload as JvmFloatArrayPayload
        val targetPayload = heap.get(target).payload as JvmFloatArrayPayload
        sourcePayload.elements[0] = Float.fromBits(0x11223344)
        sourcePayload.elements[1] = Float.fromBits(0x01020304)
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopySwapMemory0Method())
            ?: error("Unsafe.copySwapMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        assertEquals(null, result)
        assertEquals(
            listOf(Float.fromBits(0x44332211), Float.fromBits(0x04030201)),
            targetPayload.elements,
        )
    }

    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest long array element bytes`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateLongArray(2)
        val target = heap.allocateLongArray(2)
        val sourcePayload = heap.get(source).payload as JvmLongArrayPayload
        val targetPayload = heap.get(target).payload as JvmLongArrayPayload
        sourcePayload.elements[0] = 0x0102030405060708L
        sourcePayload.elements[1] = 0x0011223344556677L
        val intrinsic = JvmVmIntrinsics.Registry.resolve(unsafeCopySwapMemory0Method())
            ?: error("Unsafe.copySwapMemory0 intrinsic was not registered")
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(0L), target, JvmLongValue(0L), JvmLongValue(16L), JvmLongValue(8L)),
            ),
        )

        assertEquals(null, result)
        assertEquals(listOf(0x0807060504030201L, 0x7766554433221100L), targetPayload.elements)
    }

    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest double array element bytes`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        ...
        assertEquals(null, result)
        assertEquals(
            listOf(Double.fromBits(0x0807060504030201L), Double.fromBits(0x7766554433221100L)),
            targetPayload.elements,
        )
    }
    ...
    fun `Unsafe copySwapMemory0 intrinsic swaps synthetic native memory into guest byte arrays`() {
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(JvmNullValue, JvmLongValue(source), target, JvmLongValue(0L), JvmLongValue(4L), JvmLongValue(2L)),
            ),
        )
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps synthetic native memory into guest short arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(4L)
        ...
        val expected = if (java.nio.ByteOrder.nativeOrder() == java.nio.ByteOrder.LITTLE_ENDIAN) {
            listOf(0x0102.toShort(), 0x0304.toShort())
        } else {
            listOf(0x0201.toShort(), 0x0403.toShort())
        }
        assertEquals(null, result)
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps synthetic native memory into guest char arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(4L)
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps synthetic native memory into guest int arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(8L)
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(JvmNullValue, JvmLongValue(source), target, JvmLongValue(0L), JvmLongValue(8L), JvmLongValue(4L)),
            ),
        )

        val expected = if (java.nio.ByteOrder.nativeOrder() == java.nio.ByteOrder.LITTLE_ENDIAN) {
            listOf(0x01020304, 0x05060708)
        } else {
            listOf(0x04030201, 0x08070605)
        }
        assertEquals(null, result)
        assertEquals(expected, targetPayload.elements)
    }
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps synthetic native memory into guest float arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(8L)
        ...
        val expected = if (java.nio.ByteOrder.nativeOrder() == java.nio.ByteOrder.LITTLE_ENDIAN) {
            listOf(Float.fromBits(0x01020304), Float.fromBits(0x05060708))
        } else {
            listOf(Float.fromBits(0x04030201), Float.fromBits(0x08070605))
        }
        assertEquals(null, result)
        assertEquals(expected, targetPayload.elements)
    }
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps synthetic native memory into guest long arrays`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val memory = JvmUnsafeSyntheticMemory()
        val source = memory.allocateNativeMemory(16L)
        ...
        val expected = if (java.nio.ByteOrder.nativeOrder() == java.nio.ByteOrder.LITTLE_ENDIAN) {
            listOf(0x0102030405060708L, 0x090a0b0c0d0e0f10L)
        } else {
            listOf(0x0807060504030201L, 0x100f0e0d0c0b0a09L)
        }
        assertEquals(null, result)
        assertEquals(expected, targetPayload.elements)
    }
    ...
    fun `Unsafe copySwapMemory0 intrinsic swaps guest byte arrays into synthetic native memory`() {
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(0L), JvmNullValue, JvmLongValue(target), JvmLongValue(4L), JvmLongValue(2L)),
            ),
        )
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest short arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateShortArray(2)
        val sourcePayload = heap.get(source).payload as JvmShortArrayPayload
        ...
        val expectedBytes = if (java.nio.ByteOrder.nativeOrder() == java.nio.ByteOrder.LITTLE_ENDIAN) {
            ...
            listOf(0x22.toByte(), 0x11.toByte(), 0x44.toByte(), 0x33.toByte())
        }
        assertEquals(null, result)
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest char arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateCharArray(2)
        val sourcePayload = heap.get(source).payload as JvmCharArrayPayload
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest int arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateIntArray(2)
        val sourcePayload = heap.get(source).payload as JvmIntArrayPayload
        ...
        val result = intrinsic.invoke(
            ...
            JvmNativeMethodInvocation(
                unsafe,
                listOf(source, JvmLongValue(0L), JvmNullValue, JvmLongValue(target), JvmLongValue(8L), JvmLongValue(4L)),
            ),
        )
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest float arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateFloatArray(2)
        val sourcePayload = heap.get(source).payload as JvmFloatArrayPayload
    ...
    @Test
    fun `Unsafe copySwapMemory0 intrinsic swaps guest long arrays into synthetic native memory`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val source = heap.allocateLongArray(2)
        val sourcePayload = heap.get(source).payload as JvmLongArrayPayload
    ...
    @Test
    fun `Unsafe putFloat intrinsic writes guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("FloatFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("FloatFieldOffsetOwner")
        val fieldName = heap.internString("ratio")
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("FloatFieldOffsetOwner", "ratio", "F")
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
        ...
        val putFloat = JvmVmIntrinsics.Registry.resolve(unsafePutFloatMethod())
            ?: error("Unsafe.putFloat intrinsic was not registered")

        val result = putFloat.invoke(context, JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmFloatValue(2.5f))))
    ...
    @Test
    fun `Unsafe getDouble intrinsic reads guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        ...
        heap.putInstanceField(
            target,
            JvmFieldReference("DoubleFieldOffsetOwner", "ratio", "D"),
            JvmDoubleValue(1.25),
        )
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val getDouble = JvmVmIntrinsics.Registry.resolve(unsafeGetDoubleMethod())
            ?: error("Unsafe.getDouble intrinsic was not registered")

        val result = getDouble.invoke(context, JvmNativeMethodInvocation(unsafe, listOf(target, offset)))

        assertEquals(JvmDoubleValue(1.25), result)
    }
    ...
    @Test
    fun `Unsafe putIntVolatile intrinsic writes guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("VolatileIntFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("VolatileIntFieldOffsetOwner")
        val fieldName = heap.internString("state")
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("VolatileIntFieldOffsetOwner", "state", "I")
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
        ...
        val putIntVolatile = JvmVmIntrinsics.Registry.resolve(unsafePutIntVolatileMethod())
            ?: error("Unsafe.putIntVolatile intrinsic was not registered")

        val result = putIntVolatile.invoke(context, JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(88))))

        assertEquals(null, result)
        assertEquals(JvmIntValue(88), heap.getInstanceField(target, field))
    }
    ...
    @Test
    fun `Unsafe getBooleanVolatile intrinsic reads guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("VolatileBooleanFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("VolatileBooleanFieldOffsetOwner")
        val fieldName = heap.internString("enabled")
        val memory = JvmUnsafeSyntheticMemory()
        heap.putInstanceField(
            target,
            JvmFieldReference("VolatileBooleanFieldOffsetOwner", "enabled", "Z"),
            JvmIntValue(1),
        )
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "VolatileBooleanFieldOffsetOwner",
                        fields = listOf(JvmFieldDefinition(name = "enabled", descriptor = "Z", isStatic = false)),
                    ),
                ),
            ),
        ...
        val getBooleanVolatile = JvmVmIntrinsics.Registry.resolve(unsafeGetBooleanVolatileMethod())
            ?: error("Unsafe.getBooleanVolatile intrinsic was not registered")

        val result = getBooleanVolatile.invoke(context, JvmNativeMethodInvocation(unsafe, listOf(target, offset)))

        assertEquals(JvmIntValue(1), result)
    }

    @Test
    fun `Unsafe putBooleanVolatile intrinsic writes guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("VolatileBooleanFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("VolatileBooleanFieldOffsetOwner")
        val fieldName = heap.internString("enabled")
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("VolatileBooleanFieldOffsetOwner", "enabled", "Z")
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
        ...
        val putBooleanVolatile = JvmVmIntrinsics.Registry.resolve(unsafePutBooleanVolatileMethod())
            ?: error("Unsafe.putBooleanVolatile intrinsic was not registered")

        val result = putBooleanVolatile.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(1))),
        )
    ...
    @Test
    fun `Unsafe getByteVolatile intrinsic reads guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        ...
        heap.putInstanceField(
            target,
            JvmFieldReference("VolatileByteFieldOffsetOwner", "code", "B"),
            JvmIntValue(-7),
        )
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val getByteVolatile = JvmVmIntrinsics.Registry.resolve(unsafeGetByteVolatileMethod())
            ?: error("Unsafe.getByteVolatile intrinsic was not registered")

        val result = getByteVolatile.invoke(context, JvmNativeMethodInvocation(unsafe, listOf(target, offset)))

        assertEquals(JvmIntValue(-7), result)
    }
    ...
    @Test
    fun `Unsafe getShortVolatile intrinsic reads guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("VolatileShortFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("VolatileShortFieldOffsetOwner")
        val fieldName = heap.internString("code")
        val memory = JvmUnsafeSyntheticMemory()
        heap.putInstanceField(
            target,
            JvmFieldReference("VolatileShortFieldOffsetOwner", "code", "S"),
            JvmIntValue(-1234),
        )
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "VolatileShortFieldOffsetOwner",
                        fields = listOf(JvmFieldDefinition(name = "code", descriptor = "S", isStatic = false)),
                    ),
                ),
            ),
        ...
        val getShortVolatile = JvmVmIntrinsics.Registry.resolve(unsafeGetShortVolatileMethod())
            ?: error("Unsafe.getShortVolatile intrinsic was not registered")

        val result = getShortVolatile.invoke(context, JvmNativeMethodInvocation(unsafe, listOf(target, offset)))

        assertEquals(JvmIntValue(-1234), result)
    }

    @Test
    fun `Unsafe putShortVolatile intrinsic writes guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("VolatileShortFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("VolatileShortFieldOffsetOwner")
        val fieldName = heap.internString("code")
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("VolatileShortFieldOffsetOwner", "code", "S")
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
        ...
        val putShortVolatile = JvmVmIntrinsics.Registry.resolve(unsafePutShortVolatileMethod())
            ?: error("Unsafe.putShortVolatile intrinsic was not registered")

        val result = putShortVolatile.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(-1235))),
        )
    ...
    @Test
    fun `Unsafe getCharVolatile intrinsic reads guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("VolatileCharFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("VolatileCharFieldOffsetOwner")
        val fieldName = heap.internString("code")
        val memory = JvmUnsafeSyntheticMemory()
        heap.putInstanceField(
            target,
            JvmFieldReference("VolatileCharFieldOffsetOwner", "code", "C"),
            JvmIntValue('\u03A9'.code),
        )
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "VolatileCharFieldOffsetOwner",
        ...
        val getCharVolatile = JvmVmIntrinsics.Registry.resolve(unsafeGetCharVolatileMethod())
            ?: error("Unsafe.getCharVolatile intrinsic was not registered")

        val result = getCharVolatile.invoke(context, JvmNativeMethodInvocation(unsafe, listOf(target, offset)))

        assertEquals(JvmIntValue('\u03A9'.code), result)
    }

    @Test
    fun `Unsafe putCharVolatile intrinsic writes guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("VolatileCharFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("VolatileCharFieldOffsetOwner")
        val fieldName = heap.internString("code")
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("VolatileCharFieldOffsetOwner", "code", "C")
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
        ...
        val putCharVolatile = JvmVmIntrinsics.Registry.resolve(unsafePutCharVolatileMethod())
            ?: error("Unsafe.putCharVolatile intrinsic was not registered")

        val result = putCharVolatile.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue('\u03BB'.code))),
        )
    ...
    @Test
    fun `Unsafe putFloatVolatile intrinsic writes guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("VolatileFloatFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("VolatileFloatFieldOffsetOwner")
        val fieldName = heap.internString("ratio")
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("VolatileFloatFieldOffsetOwner", "ratio", "F")
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
        ...
        val putFloatVolatile = JvmVmIntrinsics.Registry.resolve(unsafePutFloatVolatileMethod())
            ?: error("Unsafe.putFloatVolatile intrinsic was not registered")

        val result = putFloatVolatile.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmFloatValue(2.5f))),
        )
    ...
    @Test
    fun `Unsafe getDoubleVolatile intrinsic reads guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        ...
        heap.putInstanceField(
            target,
            JvmFieldReference("VolatileDoubleFieldOffsetOwner", "ratio", "D"),
            JvmDoubleValue(1.5),
        )
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val getDoubleVolatile = JvmVmIntrinsics.Registry.resolve(unsafeGetDoubleVolatileMethod())
            ?: error("Unsafe.getDoubleVolatile intrinsic was not registered")

        val result = getDoubleVolatile.invoke(context, JvmNativeMethodInvocation(unsafe, listOf(target, offset)))

        assertEquals(JvmDoubleValue(1.5), result)
    }
    ...
    @Test
    fun `Unsafe getReferenceVolatile intrinsic reads guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        ...
        heap.putInstanceField(
            target,
            JvmFieldReference("VolatileReferenceFieldOffsetOwner", "value", "Ljava/lang/Object;"),
            reference,
        )
        val context = JvmNativeMethodContext(
            heap = heap,
        ...
        val getReferenceVolatile = JvmVmIntrinsics.Registry.resolve(unsafeGetReferenceVolatileMethod())
            ?: error("Unsafe.getReferenceVolatile intrinsic was not registered")

        val result = getReferenceVolatile.invoke(context, JvmNativeMethodInvocation(unsafe, listOf(target, offset)))
    ...
    @Test
    fun `Unsafe compareAndSetReference intrinsic updates guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        ...
        val expectedMismatch = heap.allocateObject("UnexpectedReference")
        val replacement = heap.allocateObject("ReplacementReference")
        val mismatchReplacement = heap.allocateObject("MismatchReplacementReference")
        ...
        val compareAndSetReference = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndSetReferenceMethod())
            ?: error("Unsafe.compareAndSetReference intrinsic was not registered")

        val failure = compareAndSetReference.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, expectedMismatch, mismatchReplacement),
            ),
        )
        val success = compareAndSetReference.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, initial, replacement),
            ),
        )
    ...
    @Test
    fun `Unsafe compareAndExchangeReference intrinsic returns witness for guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicReferenceFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicReferenceFieldOffsetOwner")
        ...
        val compareAndExchangeReference = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndExchangeReferenceMethod())
            ?: error("Unsafe.compareAndExchangeReference intrinsic was not registered")

        val mismatch = compareAndExchangeReference.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, expectedMismatch, mismatchReplacement),
        ...
        val match = compareAndExchangeReference.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, initial, replacement),
    ...
    fun `Unsafe compareAndSetInt intrinsic updates guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicIntFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicIntFieldOffsetOwner")
        val fieldName = heap.internString("state")
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("AtomicIntFieldOffsetOwner", "state", "I")
        heap.putInstanceField(target, field, JvmIntValue(42))
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "AtomicIntFieldOffsetOwner",
                        fields = listOf(JvmFieldDefinition(name = "state", descriptor = "I", isStatic = false)),
                    ),
                ),
            ),
        ...
        val compareAndSetInt = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndSetIntMethod())
            ?: error("Unsafe.compareAndSetInt intrinsic was not registered")

        val success = compareAndSetInt.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, JvmIntValue(42), JvmIntValue(43)),
            ),
        )
        val failure = compareAndSetInt.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, JvmIntValue(42), JvmIntValue(44)),
            ),
        )

        assertEquals(JvmIntValue(1), success)
        assertEquals(JvmIntValue(0), failure)
        assertEquals(JvmIntValue(43), heap.getInstanceField(target, field))
    }

    @Test
    fun `Unsafe compareAndExchangeInt intrinsic returns witness for guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicIntFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicIntFieldOffsetOwner")
        ...
        val compareAndExchangeInt = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndExchangeIntMethod())
            ?: error("Unsafe.compareAndExchangeInt intrinsic was not registered")

        val mismatch = compareAndExchangeInt.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, JvmIntValue(1), JvmIntValue(2)),
            ),
        )
        val match = compareAndExchangeInt.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, JvmIntValue(42), JvmIntValue(43)),
    ...
    @Test
    fun `Unsafe compareAndSetLong intrinsic updates guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicLongFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicLongFieldOffsetOwner")
        val fieldName = heap.internString("state")
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("AtomicLongFieldOffsetOwner", "state", "J")
        heap.putInstanceField(target, field, JvmLongValue(42L))
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "AtomicLongFieldOffsetOwner",
                        fields = listOf(JvmFieldDefinition(name = "state", descriptor = "J", isStatic = false)),
                    ),
                ),
            ),
        ...
        val compareAndSetLong = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndSetLongMethod())
            ?: error("Unsafe.compareAndSetLong intrinsic was not registered")

        val success = compareAndSetLong.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, JvmLongValue(42L), JvmLongValue(43L)),
            ),
        )
        val failure = compareAndSetLong.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, JvmLongValue(42L), JvmLongValue(44L)),
            ),
        )

        assertEquals(JvmIntValue(1), success)
        assertEquals(JvmIntValue(0), failure)
        assertEquals(JvmLongValue(43L), heap.getInstanceField(target, field))
    }

    @Test
    fun `Unsafe compareAndExchangeLong intrinsic returns witness for guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicLongFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicLongFieldOffsetOwner")
        ...
        val compareAndExchangeLong = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndExchangeLongMethod())
            ?: error("Unsafe.compareAndExchangeLong intrinsic was not registered")

        val mismatch = compareAndExchangeLong.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, JvmLongValue(1L), JvmLongValue(2L)),
            ),
        )
        val match = compareAndExchangeLong.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
                listOf(target, offset, JvmLongValue(42L), JvmLongValue(43L)),
    ...
    @Test
    fun `Unsafe compareAndExchangeBoolean intrinsic returns witness for guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicBooleanFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicBooleanFieldOffsetOwner")
        val fieldName = heap.internString("enabled")
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("AtomicBooleanFieldOffsetOwner", "enabled", "Z")
        heap.putInstanceField(target, field, JvmIntValue(0))
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "AtomicBooleanFieldOffsetOwner",
                        fields = listOf(JvmFieldDefinition(name = "enabled", descriptor = "Z", isStatic = false)),
                    ),
                ),
            ),
        ...
        val compareAndExchangeBoolean = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndExchangeBooleanMethod())
            ?: error("Unsafe.compareAndExchangeBoolean intrinsic was not registered")

        val mismatch = compareAndExchangeBoolean.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(1), JvmIntValue(1))),
        )
        val match = compareAndExchangeBoolean.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(0), JvmIntValue(1))),
        )

        assertEquals(JvmIntValue(0), mismatch)
        assertEquals(JvmIntValue(0), match)
        assertEquals(JvmIntValue(1), heap.getInstanceField(target, field))
    }

    @Test
    fun `Unsafe compareAndExchangeByte intrinsic returns witness for guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        ...
        val compareAndExchangeByte = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndExchangeByteMethod())
            ?: error("Unsafe.compareAndExchangeByte intrinsic was not registered")

        val mismatch = compareAndExchangeByte.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(8), JvmIntValue(9))),
        )
        val match = compareAndExchangeByte.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(7), JvmIntValue(-8))),
        )

        assertEquals(JvmIntValue(7), mismatch)
        assertEquals(JvmIntValue(7), match)
        assertEquals(JvmIntValue(-8), heap.getInstanceField(target, field))
    }
    ...
    @Test
    fun `Unsafe compareAndExchangeChar intrinsic returns witness for guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        ...
        val compareAndExchangeChar = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndExchangeCharMethod())
            ?: error("Unsafe.compareAndExchangeChar intrinsic was not registered")

        val mismatch = compareAndExchangeChar.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue('A'.code), JvmIntValue('B'.code))),
        )
        val match = compareAndExchangeChar.invoke(
            context,
            JvmNativeMethodInvocation(
                unsafe,
        ...
        assertEquals(JvmIntValue('\u03A9'.code), mismatch)
        assertEquals(JvmIntValue('\u03A9'.code), match)
        assertEquals(JvmIntValue('\u03BB'.code), heap.getInstanceField(target, field))
    }
    ...
    @Test
    fun `Unsafe compareAndExchangeDouble intrinsic returns witness for guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicDoubleFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicDoubleFieldOffsetOwner")
        val fieldName = heap.internString("ratio")
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("AtomicDoubleFieldOffsetOwner", "ratio", "D")
        heap.putInstanceField(target, field, JvmDoubleValue(1.25))
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "AtomicDoubleFieldOffsetOwner",
                        fields = listOf(JvmFieldDefinition(name = "ratio", descriptor = "D", isStatic = false)),
                    ),
                ),
            ),
        ...
        val compareAndExchangeDouble = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndExchangeDoubleMethod())
            ?: error("Unsafe.compareAndExchangeDouble intrinsic was not registered")

        val mismatch = compareAndExchangeDouble.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmDoubleValue(2.5), JvmDoubleValue(3.5))),
        )
        val match = compareAndExchangeDouble.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmDoubleValue(1.25), JvmDoubleValue(2.5))),
        )

        assertEquals(JvmDoubleValue(1.25), mismatch)
        assertEquals(JvmDoubleValue(1.25), match)
        assertEquals(JvmDoubleValue(2.5), heap.getInstanceField(target, field))
    }
    ...
    fun `Unsafe getAndAddLong intrinsic returns witness and adds guest object fields through synthetic offsets`() {
        ...
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("AtomicLongFieldOffsetOwner", "state", "J")
        heap.putInstanceField(target, field, JvmLongValue(40L))
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
        ...
        val getAndAddLong = JvmVmIntrinsics.Registry.resolve(unsafeGetAndAddLongMethod())
            ?: error("Unsafe.getAndAddLong intrinsic was not registered")

        val witness = getAndAddLong.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmLongValue(2L))),
        )

        assertEquals(JvmLongValue(40L), witness)
        assertEquals(JvmLongValue(42L), heap.getInstanceField(target, field))
    }
    @Test
    fun `Unsafe getAndSetLong intrinsic returns witness and replaces guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicLongFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicLongFieldOffsetOwner")
        ...
        val getAndSetLong = JvmVmIntrinsics.Registry.resolve(unsafeGetAndSetLongMethod())
            ?: error("Unsafe.getAndSetLong intrinsic was not registered")

        val witness = getAndSetLong.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmLongValue(84L))),
        )

        assertEquals(JvmLongValue(42L), witness)
        assertEquals(JvmLongValue(84L), heap.getInstanceField(target, field))
    }
    @Test
    fun `Unsafe getAndAddInt intrinsic returns witness and adds guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicIntFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicIntFieldOffsetOwner")
        ...
        val memory = JvmUnsafeSyntheticMemory()
        val field = JvmFieldReference("AtomicIntFieldOffsetOwner", "state", "I")
        heap.putInstanceField(target, field, JvmIntValue(40))
        val context = JvmNativeMethodContext(
            heap = heap,
            classHierarchy = JvmClassHierarchy(
                listOf(
        ...
        val getAndAddInt = JvmVmIntrinsics.Registry.resolve(unsafeGetAndAddIntMethod())
            ?: error("Unsafe.getAndAddInt intrinsic was not registered")

        val witness = getAndAddInt.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(2))),
        )

        assertEquals(JvmIntValue(40), witness)
        assertEquals(JvmIntValue(42), heap.getInstanceField(target, field))
    }
    @Test
    fun `Unsafe getAndSetInt intrinsic returns witness and replaces guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicIntFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicIntFieldOffsetOwner")
        ...
        val getAndSetInt = JvmVmIntrinsics.Registry.resolve(unsafeGetAndSetIntMethod())
            ?: error("Unsafe.getAndSetInt intrinsic was not registered")

        val witness = getAndSetInt.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(84))),
        )

        assertEquals(JvmIntValue(42), witness)
        assertEquals(JvmIntValue(84), heap.getInstanceField(target, field))
    }

    @Test
    fun `Unsafe getAndSetBoolean intrinsic returns witness and replaces guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicBooleanFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicBooleanFieldOffsetOwner")
        ...
        val getAndSetBoolean = JvmVmIntrinsics.Registry.resolve(unsafeGetAndSetBooleanMethod())
            ?: error("Unsafe.getAndSetBoolean intrinsic was not registered")

        val witness = getAndSetBoolean.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(1))),
        )

        assertEquals(JvmIntValue(0), witness)
        assertEquals(JvmIntValue(1), heap.getInstanceField(target, field))
    }
    ...
    @Test
    fun `Unsafe getAndSetShort intrinsic returns witness and replaces guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicShortFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicShortFieldOffsetOwner")
        ...
        val getAndSetShort = JvmVmIntrinsics.Registry.resolve(unsafeGetAndSetShortMethod())
            ?: error("Unsafe.getAndSetShort intrinsic was not registered")

        val witness = getAndSetShort.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(-1235))),
        )

        assertEquals(JvmIntValue(1234), witness)
        assertEquals(JvmIntValue(-1235), heap.getInstanceField(target, field))
    }

    @Test
    fun `Unsafe getAndSetChar intrinsic returns witness and replaces guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicCharFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicCharFieldOffsetOwner")
        ...
        val getAndSetChar = JvmVmIntrinsics.Registry.resolve(unsafeGetAndSetCharMethod())
            ?: error("Unsafe.getAndSetChar intrinsic was not registered")

        val witness = getAndSetChar.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue('\u03BB'.code))),
        )

        assertEquals(JvmIntValue('\u03A9'.code), witness)
        assertEquals(JvmIntValue('\u03BB'.code), heap.getInstanceField(target, field))
    }

    @Test
    fun `Unsafe getAndSetFloat intrinsic returns witness and replaces guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicFloatFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicFloatFieldOffsetOwner")
        ...
        val getAndSetFloat = JvmVmIntrinsics.Registry.resolve(unsafeGetAndSetFloatMethod())
            ?: error("Unsafe.getAndSetFloat intrinsic was not registered")

        val witness = getAndSetFloat.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmFloatValue(2.5f))),
        )

        assertEquals(JvmFloatValue(1.25f), witness)
        assertEquals(JvmFloatValue(2.5f), heap.getInstanceField(target, field))
    }

    @Test
    fun `Unsafe getAndSetDouble intrinsic returns witness and replaces guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicDoubleFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicDoubleFieldOffsetOwner")
        ...
        val getAndSetDouble = JvmVmIntrinsics.Registry.resolve(unsafeGetAndSetDoubleMethod())
            ?: error("Unsafe.getAndSetDouble intrinsic was not registered")

        val witness = getAndSetDouble.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmDoubleValue(2.5))),
        )

        assertEquals(JvmDoubleValue(1.25), witness)
        assertEquals(JvmDoubleValue(2.5), heap.getInstanceField(target, field))
    }
    ...
    fun `Unsafe compareAndSetBoolean intrinsic updates guest object fields through synthetic offsets`() {
        ...
        val compareAndSetBoolean = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndSetBooleanMethod())
            ?: error("Unsafe.compareAndSetBoolean intrinsic was not registered")

        val failure = compareAndSetBoolean.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(1), JvmIntValue(0))),
        )
        val success = compareAndSetBoolean.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(0), JvmIntValue(1))),
        )
    ...
    @Test
    fun `Unsafe compareAndSetByte intrinsic updates guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicByteFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicByteFieldOffsetOwner")
        ...
        val compareAndSetByte = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndSetByteMethod())
            ?: error("Unsafe.compareAndSetByte intrinsic was not registered")

        val failure = compareAndSetByte.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(8), JvmIntValue(9))),
        )
        val success = compareAndSetByte.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(7), JvmIntValue(-8))),
        )
    ...
    @Test
    fun `Unsafe compareAndSetShort intrinsic updates guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicShortFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicShortFieldOffsetOwner")
        ...
        val compareAndSetShort = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndSetShortMethod())
            ?: error("Unsafe.compareAndSetShort intrinsic was not registered")

        val failure = compareAndSetShort.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(7), JvmIntValue(8))),
        )
        val success = compareAndSetShort.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmIntValue(1234), JvmIntValue(-1235))),
        )
    ...
    @Test
    fun `Unsafe compareAndSetDouble intrinsic updates guest object fields through synthetic offsets`() {
        val heap = JvmHeap()
        val unsafe = heap.allocateObject("jdk/internal/misc/Unsafe")
        val target = heap.allocateObject("AtomicDoubleFieldOffsetOwner")
        val ownerClass = heap.internClassMirror("AtomicDoubleFieldOffsetOwner")
        ...
        val compareAndSetDouble = JvmVmIntrinsics.Registry.resolve(unsafeCompareAndSetDoubleMethod())
            ?: error("Unsafe.compareAndSetDouble intrinsic was not registered")

        val failure = compareAndSetDouble.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmDoubleValue(2.5), JvmDoubleValue(3.5))),
        )
        val success = compareAndSetDouble.invoke(
            context,
            JvmNativeMethodInvocation(unsafe, listOf(target, offset, JvmDoubleValue(1.25), JvmDoubleValue(2.5))),
        )
    ...
    private fun classGetNestHost0Method(): JvmResolvedMethod = JvmResolvedMethod(
        ownerClassName = "java/lang/Class",
        name = "getNestHost0",
        descriptor = "()Ljava/lang/Class;",
        isStatic = false,
    ...
    private fun classGetComponentTypeMethod(): JvmResolvedMethod = JvmResolvedMethod(
        ownerClassName = "java/lang/Class",
    ...
    private fun classGetModifiersMethod(): JvmResolvedMethod = JvmResolvedMethod(
        ownerClassName = "java/lang/Class",
        name = "getModifiers",
        descriptor = "()I",
        isStatic = false,
