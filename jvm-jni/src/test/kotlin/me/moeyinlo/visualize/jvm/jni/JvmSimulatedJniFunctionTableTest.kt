...
class JvmSimulatedJniFunctionTableTest {
    ...
    fun `function table delegates class member lookup helpers to one simulated JNI environment`() {
        ...
        val moduleHandle = functions.getModule(classHandle)
        ...
        assertEquals("java/lang/Module", heap.get(handles.resolveObject(moduleHandle)).className)
    ...
    @Test
    fun `function table delegates nonvirtual object method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableObjectUpcall>()
        val resultReference = heap.allocateObject("java/lang/Object")
        ...
        val receiver = heap.allocateObject("Derived")
        val objectHandle = handles.newObjectHandle(receiver)
        val methodHandle = functions.getMethodId(baseClassHandle, "pick", "()Ljava/lang/Object;")

        val resultHandle = functions.callNonvirtualObjectMethod(
            objectHandle,
            baseClassHandle,
            methodHandle,
            emptyList(),
    ...
    @Test
    fun `function table delegates nonvirtual boolean method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableBooleanUpcall>()
        val environment = JvmSimulatedJniEnvironment(
        ...
        val receiver = heap.allocateObject("Derived")
        val objectHandle = handles.newObjectHandle(receiver)
        val methodHandle = functions.getMethodId(baseClassHandle, "enabled", "()Z")

        val result = functions.callNonvirtualBooleanMethod(objectHandle, baseClassHandle, methodHandle, emptyList())
    ...
    @Test
    fun `function table delegates nonvirtual char method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableCharUpcall>()
        val environment = JvmSimulatedJniEnvironment(
        ...
        val receiver = heap.allocateObject("Derived")
        val objectHandle = handles.newObjectHandle(receiver)
        val methodHandle = functions.getMethodId(baseClassHandle, "letter", "()C")

        val result = functions.callNonvirtualCharMethod(objectHandle, baseClassHandle, methodHandle, emptyList())

        assertEquals('r'.code, result)
        assertEquals(
            listOf(
                RecordedFunctionTableCharUpcall(
                    receiver = receiver,
    ...
    @Test
    fun `function table delegates nonvirtual int method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableIntUpcall>()
        val environment = JvmSimulatedJniEnvironment(
        ...
        val receiver = heap.allocateObject("Derived")
        val objectHandle = handles.newObjectHandle(receiver)
        val methodHandle = functions.getMethodId(baseClassHandle, "answer", "()I")

        val result = functions.callNonvirtualIntMethod(objectHandle, baseClassHandle, methodHandle, emptyList())

        assertEquals(123457, result)
        assertEquals(
            listOf(
                RecordedFunctionTableIntUpcall(
                    receiver = receiver,
    ...
    @Test
    fun `function table delegates long method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableLongUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "wide", descriptor = "()J", isStatic = false),
                        ),
                    ),
                ),
            ),
            ...
            upcallDispatcher = object : JvmJniUpcallDispatcher {
                ...
                override fun callLongMethod(
                    ...
                    calls += RecordedFunctionTableLongUpcall(receiver, method, arguments)
        ...
        val receiver = heap.allocateObject("Example")
        val objectHandle = handles.newObjectHandle(receiver)
        val methodHandle = functions.getMethodId(classHandle, "wide", "()J")

        val result = functions.callLongMethod(objectHandle, methodHandle, emptyList())

        assertEquals(9_876_543_210L, result)
        assertEquals(
            listOf(
                RecordedFunctionTableLongUpcall(
                    receiver = receiver,
                    method = JvmResolvedMethod(
                        ownerClassName = "Example",
                        name = "wide",
    ...
    @Test
    fun `function table delegates nonvirtual long method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableLongUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Base",
                        methods = listOf(
                            JvmMethodDefinition(name = "counter", descriptor = "()J", isStatic = false),
                        ),
                    ),
                    JvmClassDefinition(
                        internalName = "Derived",
            ...
            upcallDispatcher = object : JvmJniUpcallDispatcher {
                ...
                override fun callLongMethod(
                    ...
                    calls += RecordedFunctionTableLongUpcall(receiver, method, arguments)
                    return JvmLongValue(9876543211L)
                }
            },
        )
        val functions = environment.functions
        ...
        val receiver = heap.allocateObject("Derived")
        val objectHandle = handles.newObjectHandle(receiver)
        val methodHandle = functions.getMethodId(baseClassHandle, "counter", "()J")

        val result = functions.callNonvirtualLongMethod(objectHandle, baseClassHandle, methodHandle, emptyList())
    ...
    @Test
    fun `function table delegates nonvirtual float method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableFloatUpcall>()
        val environment = JvmSimulatedJniEnvironment(
        ...
        val receiver = heap.allocateObject("Derived")
        val objectHandle = handles.newObjectHandle(receiver)
        val methodHandle = functions.getMethodId(baseClassHandle, "ratio", "()F")

        val result = functions.callNonvirtualFloatMethod(objectHandle, baseClassHandle, methodHandle, emptyList())
    ...
    @Test
    fun `function table delegates double method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableDoubleUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "score", descriptor = "()D", isStatic = false),
                        ),
                    ),
                ),
            ),
            ...
            upcallDispatcher = object : JvmJniUpcallDispatcher {
                ...
                override fun callDoubleMethod(
                    ...
                    calls += RecordedFunctionTableDoubleUpcall(receiver, method, arguments)
                    return JvmDoubleValue(42.5)
                }
            },
        )
        val functions = environment.functions
        ...
        val receiver = heap.allocateObject("Example")
        val objectHandle = handles.newObjectHandle(receiver)
        val methodHandle = functions.getMethodId(classHandle, "score", "()D")

        val result = functions.callDoubleMethod(objectHandle, methodHandle, emptyList())

        assertEquals(42.5, result)
        assertEquals(
            listOf(
                RecordedFunctionTableDoubleUpcall(
                    receiver = receiver,
                    method = JvmResolvedMethod(
                        ownerClassName = "Example",
                        name = "score",
    ...
    @Test
    fun `function table delegates nonvirtual double method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableDoubleUpcall>()
        val environment = JvmSimulatedJniEnvironment(
        ...
        val receiver = heap.allocateObject("Derived")
        val objectHandle = handles.newObjectHandle(receiver)
        val methodHandle = functions.getMethodId(baseClassHandle, "score", "()D")

        val result = functions.callNonvirtualDoubleMethod(objectHandle, baseClassHandle, methodHandle, emptyList())
    ...
    @Test
    fun `function table delegates static void method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableStaticVoidUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "boot", descriptor = "(I)V", isStatic = true),
                        ),
                    ),
                ),
            ),
            ...
            upcallDispatcher = object : JvmJniUpcallDispatcher {
                ...
                override fun callStaticVoidMethod(
                    ...
                    calls += RecordedFunctionTableStaticVoidUpcall(method, arguments)
                }
            },
        )
        val functions = environment.functions
        val classHandle = functions.findClass("Example")
        val methodHandle = functions.getStaticMethodId(classHandle, "boot", "(I)V")

        functions.callStaticVoidMethod(classHandle, methodHandle, listOf(JvmIntValue(11)))

        assertEquals(
            listOf(
                RecordedFunctionTableStaticVoidUpcall(
                    method = JvmResolvedMethod(
                        ownerClassName = "Example",
                        name = "boot",
                        descriptor = "(I)V",
    ...
    @Test
    fun `function table delegates static object method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableStaticObjectUpcall>()
        val resultReference = heap.allocateObject("java/lang/Object")
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    ...
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "pick", descriptor = "(I)Ljava/lang/Object;", isStatic = true),
                        ),
                    ),
                ),
            ),
        ...
        val functions = environment.functions
        val classHandle = functions.findClass("Example")
        val methodHandle = functions.getStaticMethodId(classHandle, "pick", "(I)Ljava/lang/Object;")

        val resultHandle = functions.callStaticObjectMethod(classHandle, methodHandle, listOf(JvmIntValue(13)))

        assertEquals(resultReference, handles.resolveObject(resultHandle!!))
        assertEquals(
            listOf(
                RecordedFunctionTableStaticObjectUpcall(
                    method = JvmResolvedMethod(
                        ownerClassName = "Example",
                        name = "pick",
    ...
    @Test
    fun `function table delegates static boolean method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableStaticBooleanUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "enabled", descriptor = "(I)Z", isStatic = true),
                        ),
                    ),
                ),
            ),
        ...
        val functions = environment.functions
        val classHandle = functions.findClass("Example")
        val methodHandle = functions.getStaticMethodId(classHandle, "enabled", "(I)Z")

        val result = functions.callStaticBooleanMethod(classHandle, methodHandle, listOf(JvmIntValue(15)))

        assertEquals(true, result)
        assertEquals(
            listOf(
                RecordedFunctionTableStaticBooleanUpcall(
                    method = JvmResolvedMethod(
                        ownerClassName = "Example",
                        name = "enabled",
                        descriptor = "(I)Z",
    ...
    @Test
    fun `function table delegates static byte method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableStaticByteUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "code", descriptor = "(I)B", isStatic = true),
                        ),
                    ),
                ),
            ),
        ...
        val functions = environment.functions
        val classHandle = functions.findClass("Example")
        val methodHandle = functions.getStaticMethodId(classHandle, "code", "(I)B")

        val result = functions.callStaticByteMethod(classHandle, methodHandle, listOf(JvmIntValue(17)))

        assertEquals(-7, result)
        assertEquals(
            listOf(
                RecordedFunctionTableStaticByteUpcall(
                    method = JvmResolvedMethod(
                        ownerClassName = "Example",
                        name = "code",
                        descriptor = "(I)B",
    ...
    @Test
    fun `function table delegates static char method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableStaticCharUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "letter", descriptor = "(I)C", isStatic = true),
                        ),
                    ),
                ),
            ),
        ...
        val functions = environment.functions
        val classHandle = functions.findClass("Example")
        val methodHandle = functions.getStaticMethodId(classHandle, "letter", "(I)C")

        val result = functions.callStaticCharMethod(classHandle, methodHandle, listOf(JvmIntValue(19)))

        assertEquals('z'.code, result)
        assertEquals(
            listOf(
                RecordedFunctionTableStaticCharUpcall(
                    method = JvmResolvedMethod(
                        ownerClassName = "Example",
                        name = "letter",
                        descriptor = "(I)C",
    ...
    @Test
    fun `function table delegates static short method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableStaticShortUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "delta", descriptor = "(I)S", isStatic = true),
                        ),
                    ),
                ),
            ),
        ...
        val functions = environment.functions
        val classHandle = functions.findClass("Example")
        val methodHandle = functions.getStaticMethodId(classHandle, "delta", "(I)S")

        val result = functions.callStaticShortMethod(classHandle, methodHandle, listOf(JvmIntValue(21)))

        assertEquals(-321, result)
        assertEquals(
            listOf(
                RecordedFunctionTableStaticShortUpcall(
                    method = JvmResolvedMethod(
                        ownerClassName = "Example",
                        name = "delta",
                        descriptor = "(I)S",
    ...
    @Test
    fun `function table delegates static long method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableStaticLongUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "wide", descriptor = "(I)J", isStatic = true),
                        ),
                    ),
                ),
            ),
        ...
        val functions = environment.functions
        val classHandle = functions.findClass("Example")
        val methodHandle = functions.getStaticMethodId(classHandle, "wide", "(I)J")

        val result = functions.callStaticLongMethod(classHandle, methodHandle, listOf(JvmIntValue(29)))

        assertEquals(1_234_567_890_123L, result)
        assertEquals(
            listOf(
                RecordedFunctionTableStaticLongUpcall(
                    method = JvmResolvedMethod(
                        ownerClassName = "Example",
                        name = "wide",
                        descriptor = "(I)J",
    ...
    @Test
    fun `function table delegates static double method upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableStaticDoubleUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "score", descriptor = "(I)D", isStatic = true),
                        ),
                    ),
                ),
            ),
        ...
        val functions = environment.functions
        val classHandle = functions.findClass("Example")
        val methodHandle = functions.getStaticMethodId(classHandle, "score", "(I)D")

        val result = functions.callStaticDoubleMethod(classHandle, methodHandle, listOf(JvmIntValue(37)))

        assertEquals(99.5, result)
        assertEquals(
            listOf(
                RecordedFunctionTableStaticDoubleUpcall(
                    method = JvmResolvedMethod(
                        ownerClassName = "Example",
                        name = "score",
                        descriptor = "(I)D",
                    ...
                    arguments = listOf(JvmIntValue(37)),
                ),
            ),
            calls,
        )
    ...
    fun `function table delegates NewObject constructor upcalls to one simulated JNI environment`() {
        val heap = JvmHeap()
        val handles = JvmJniHandleTable()
        val calls = mutableListOf<RecordedFunctionTableNewObjectUpcall>()
        val environment = JvmSimulatedJniEnvironment(
            classHierarchy = JvmClassHierarchy(
                listOf(
                    JvmClassDefinition(
                        internalName = "Example",
                        methods = listOf(
                            JvmMethodDefinition(name = "<init>", descriptor = "(I)V", isStatic = false),
                        ),
                    ),
                ),
            ),
            ...
            upcallDispatcher = object : JvmJniUpcallDispatcher {
                override fun callVoidMethod(
                    ...
                    calls += RecordedFunctionTableNewObjectUpcall(receiver, method, arguments)
                }
            },
        )
        val functions = environment.functions
        val classHandle = functions.findClass("Example")
        val constructorHandle = functions.getMethodId(classHandle, "<init>", "(I)V")

        val objectHandle = functions.newObject(classHandle, constructorHandle, listOf(JvmIntValue(43)))
        val objectReference = handles.resolveObject(objectHandle)

        assertEquals("Example", heap.get(objectReference).className)
        assertEquals(
            listOf(
                RecordedFunctionTableNewObjectUpcall(
                    ...
                    arguments = listOf(JvmIntValue(43)),
                ),
            ),
            calls,
        )
    ...
    fun `function table delegates exception helpers to one simulated JNI environment`() {
        ...
        assertEquals(true, functions.exceptionCheck())
        assertEquals(pendingReference, handles.resolveObject(functions.exceptionOccurred()!!))
    ...
    fun `function table delegates string helpers to one simulated JNI environment`() {
        ...
        assertContentEquals(charArrayOf('J', 'V'), functions.getStringRegion(utfHandle, 1, 2))
        assertContentEquals(
            byteArrayOf(0xc0.toByte(), 0x80.toByte(), 0x4a, 0x56, 0x4d),
...
private data class RecordedFunctionTableDoubleUpcall(
    val receiver: JvmObjectReferenceValue,
    val method: JvmResolvedMethod,
    val arguments: List<JvmValue>,
)

private data class RecordedFunctionTableStaticVoidUpcall(
    val method: JvmResolvedMethod,
    val arguments: List<JvmValue>,
)

private data class RecordedFunctionTableStaticObjectUpcall(
    val method: JvmResolvedMethod,
    val arguments: List<JvmValue>,
)

private data class RecordedFunctionTableStaticBooleanUpcall(
    val method: JvmResolvedMethod,
    val arguments: List<JvmValue>,
)

private data class RecordedFunctionTableStaticByteUpcall(
    val method: JvmResolvedMethod,
    val arguments: List<JvmValue>,
)

private data class RecordedFunctionTableStaticCharUpcall(
    val method: JvmResolvedMethod,
    val arguments: List<JvmValue>,
)
...
private data class RecordedFunctionTableStaticDoubleUpcall(
    val method: JvmResolvedMethod,
    val arguments: List<JvmValue>,
)

private data class RecordedFunctionTableNewObjectUpcall(
    val receiver: JvmObjectReferenceValue,
    val method: JvmResolvedMethod,
    val arguments: List<JvmValue>,
)
