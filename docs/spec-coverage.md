...
| Invocation, 5.4.3.6, 6.5 | Runtime model stores linked `invokedynamic` call sites by defining class and bytecode offset, preserves constant-pool index, bootstrap method index, name, and descriptor, models guest `java/lang/invoke/CallSite` heap objects with target `MethodHandle` payloads, validates bootstrap `CallSite` return objects, extracts their target `MethodHandle`, resolves `InvokeStatic` MethodHandle targets through the class hierarchy, and rejects rebinding an already linked call site | `jvm-runtime` | `JvmInvokeDynamicCallSiteRegistryTest`, `JvmHeapTest.heap allocates call site objects with target method handle payloads`, `JvmHeapTest.heap rejects call site targets that are not method handles`, `JvmHeapTest.heap shallow clones call site payloads while preserving target identity`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap invocation extracts call site target method handle from bootstrap result`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap invocation rejects non call site bootstrap results`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap invocation rejects call sites without target payloads`, `JvmInvokeDynamicCallSiteRegistryTest.method handle resolver resolves invoke static targets through the class hierarchy`, `JvmInvokeDynamicCallSiteRegistryTest.method handle resolver keeps unsupported reference kinds explicit` | IMPLEMENTED |
