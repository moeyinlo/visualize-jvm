...
| Invocation | `invokevirtual`, `invokespecial`, and `invokestatic` execute interpreted and native call paths; `invokeinterface` now performs runtime `count` and fourth-zero operand validation before the still-pending interface dispatch path; `invokedynamic` remains pending | `jvm-interpreter`, `jvm-runtime`, `jvm-verifier` | `JvmInterpreterTest.invokeinterface rejects count that does not match descriptor slots`, `JvmInterpreterTest.invokeinterface rejects nonzero fourth operand` | PENDING |
