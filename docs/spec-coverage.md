...
| Invocation, 5.4.3.6, 6.5 | `invokedynamic` execution validates the reserved zero operand bytes, rejects non-`CONSTANT_InvokeDynamic_info` entries, resolves call-site name, descriptor, bootstrap index, bootstrap method handle, and static bootstrap arguments, and materializes the bootstrap method guest argument list (`Lookup`, name `String`, call-site `MethodType`, resolved static arguments) before the pending bootstrap-execution path | `jvm-interpreter`, `jvm-runtime` | `JvmInterpreterTest.invokedynamic rejects nonzero third operand`, `JvmInterpreterTest.invokedynamic rejects nonzero fourth operand`, `JvmInterpreterTest.invokedynamic rejects non invoke dynamic constant pool entries`, `JvmInterpreterTest.invokedynamic materializes bootstrap method guest arguments before bootstrap execution`, `JvmInterpreterTest.invokedynamic reports missing bootstrap method table entries`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap invocation materializes lookup name type and resolved static arguments as guest values`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap invocation leaves dynamic static arguments to dynamic constant resolution` | IMPLEMENTED |
