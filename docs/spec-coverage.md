...
| Object and arrays | `new`, `newarray`, `anewarray`, `arraylength`; `multianewarray` now rejects zero dimensions and converts negative dimension counts to guest `NegativeArraySizeException`, while positive-dimension recursive allocation remains TBD | `jvm-interpreter`, `jvm-runtime`, `jvm-verifier` | `JvmInterpreterTest.multianewarray rejects zero dimensions`, `JvmInterpreterTest.multianewarray throws guest NegativeArraySizeException for negative counts` | PENDING |
