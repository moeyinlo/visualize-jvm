...
| Invocation | `invokevirtual`, `invokespecial`, and `invokestatic` execute interpreted and native call paths; `invokeinterface` now performs runtime `count` and fourth-zero operand validation and executes receiver-class concrete interpreted/native implementations plus direct interface default methods, while maximally-specific interface conflict selection remains pending; `invokedynamic` remains pending | `jvm-interpreter`, `jvm-runtime`, `jvm-verifier` | `JvmInterpreterTest.invokeinterface rejects count that does not match descriptor slots`, `JvmInterpreterTest.invokeinterface rejects nonzero fourth operand`, `JvmInterpreterTest.invokeinterface executes receiver class implementation`, `JvmInterpreterTest.invokeinterface executes receiver class native implementation`, `JvmInterpreterTest.invokeinterface executes direct interface default method` | PENDING |
