...
| Invocation, 5.4.3.6, 6.5 | Runtime model stores linked `invokedynamic` call sites by defining class and bytecode offset, preserves constant-pool index, bootstrap method index, name, descriptor, and target `MethodHandle` payload, models guest `java/lang/invoke/CallSite` heap objects with target `MethodHandle` payloads, validates bootstrap `CallSite` return objects, extracts their target `MethodHandle`, resolves `InvokeStatic` and `InvokeVirtual` MethodHandle targets through the class hierarchy, binds bootstrap results into the linked call-site registry, and rejects rebinding an already linked call site | `jvm-runtime` | `JvmInvokeDynamicCallSiteRegistryTest`, `JvmHeapTest.heap allocates call site objects with target method handle payloads`, `JvmHeapTest.heap rejects call site targets that are not method handles`, `JvmHeapTest.heap shallow clones call site payloads while preserving target identity`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap invocation extracts call site target method handle from bootstrap result`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap invocation rejects non call site bootstrap results`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap invocation rejects call sites without target payloads`, `JvmInvokeDynamicCallSiteRegistryTest.method handle resolver resolves invoke static targets through the class hierarchy`, `JvmInvokeDynamicCallSiteRegistryTest.method handle resolver resolves invoke virtual targets through the class hierarchy`, `JvmInvokeDynamicCallSiteRegistryTest.method handle resolver rejects invoke virtual targets that resolve to static methods`, `JvmInvokeDynamicCallSiteRegistryTest.method handle resolver keeps unsupported reference kinds explicit`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap result binding caches invokedynamic target methods by call site key` | IMPLEMENTED |
...
| Invocation, 5.4.3.6, 6.5 | `invokedynamic` execution validates the reserved zero operand bytes, rejects non-`CONSTANT_InvokeDynamic_info` entries, dispatches already-linked cached call sites by defining class and bytecode offset to static interpreted/native targets, passes descriptor arguments through the shared static invocation path, rejects cached targets whose descriptor does not match the linked call-site descriptor, resolves call-site name, descriptor, bootstrap index, bootstrap method handle, and static bootstrap arguments on cache misses, materializes the bootstrap method guest argument list (`Lookup`, name `String`, call-site `MethodType`, resolved static arguments), executes `InvokeStatic` bootstrap method handles through interpreted/native static invocation, validates guest `CallSite` bootstrap results, binds the returned target `MethodHandle`, dispatches the first linked target execution, and keeps non-`InvokeStatic` linked target MethodHandle execution as an explicit pending dispatch path | `jvm-interpreter`, `jvm-runtime` | `JvmInterpreterTest.invokedynamic rejects nonzero third operand`, `JvmInterpreterTest.invokedynamic rejects nonzero fourth operand`, `JvmInterpreterTest.invokedynamic rejects non invoke dynamic constant pool entries`, `JvmInterpreterTest.invokedynamic executes cached linked static call site target`, `JvmInterpreterTest.invokedynamic passes descriptor arguments to cached linked static targets`, `JvmInterpreterTest.invokedynamic rejects cached linked targets with mismatched descriptors`, `JvmInterpreterTest.invokedynamic keeps non static linked target method handles explicit`, `JvmInterpreterTest.invokedynamic materializes bootstrap method guest arguments before bootstrap execution`, `JvmInterpreterTest.invokedynamic executes invoke static bootstrap method and dispatches linked target`, `JvmInterpreterTest.invokedynamic rejects bootstrap results that are not call sites`, `JvmInterpreterTest.invokedynamic reports missing bootstrap method table entries`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap invocation materializes lookup name type and resolved static arguments as guest values`, `JvmInvokeDynamicCallSiteRegistryTest.bootstrap invocation leaves dynamic static arguments to dynamic constant resolution` | IMPLEMENTED |
