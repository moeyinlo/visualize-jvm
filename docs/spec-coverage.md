...
| Invocation | `invokevirtual`, `invokespecial`, and `invokestatic` execute interpreted and native call paths; `invokeinterface` now performs runtime `count` and fourth-zero operand validation and executes receiver-class concrete implementations, while default/maximally-specific interface selection and native interface targets remain pending; `invokedynamic` remains pending | `jvm-interpreter`, `jvm-runtime`, `jvm-verifier` | `JvmInterpreterTest.invokeinterface rejects count that does not match descriptor slots`, `JvmInterpreterTest.invokeinterface rejects nonzero fourth operand`, `JvmInterpreterTest.invokeinterface executes receiver class implementation` | PENDING |
