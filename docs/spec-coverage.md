...
| Object and arrays | `new`, `newarray`, `anewarray`, `arraylength`; `multianewarray` now rejects zero dimensions, converts negative dimension counts to guest `NegativeArraySizeException`, and allocates one-dimensional arrays; recursive multi-dimensional allocation remains TBD | `jvm-interpreter`, `jvm-runtime`, `jvm-verifier` | `JvmInterpreterTest.multianewarray rejects zero dimensions`, `JvmInterpreterTest.multianewarray throws guest NegativeArraySizeException for negative counts`, `JvmInterpreterTest.multianewarray allocates one dimensional primitive arrays` | PENDING |
