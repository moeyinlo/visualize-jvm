...
| Invocation, 4.7.23, 5.4.3.6 | `invokedynamic` runtime resolver links each call-site spec to its bootstrap method table entry using the zero-based `bootstrap_method_attr_index` and reports missing bootstrap entries explicitly | `jvm-runtime` | `JvmInvokeDynamicCallSiteRegistryTest.call site resolver links invoke dynamic specs to bootstrap methods by zero based index`, `JvmInvokeDynamicCallSiteRegistryTest.call site resolver rejects missing bootstrap method table entries` | IMPLEMENTED || Invocation, 5.4.3.6, 6.5 | `invokedynamic` execution validates the reserved zero operand bytes, rejects non-`CONSTANT_InvokeDynamic_info` entries, and resolves call-site name, descriptor, and bootstrap index before the pending bootstrap-linkage path | `jvm-interpreter`, `jvm-runtime` | `JvmInterpreterTest.invokedynamic rejects nonzero third operand`, `JvmInterpreterTest.invokedynamic rejects nonzero fourth operand`, `JvmInterpreterTest.invokedynamic rejects non invoke dynamic constant pool entries`, `JvmInterpreterTest.invokedynamic resolves call site spec before bootstrap linkage` | IMPLEMENTED || Method resolution, 5.4.3.3 | Class method resolution finds a method declared directly by the referenced class and reports guest `NoSuchMethodError` on misses | `jvm-runtime` | `JvmMethodResolutionTest` | IMPLEMENTED |
