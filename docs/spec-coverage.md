...
| Invocation, 5.4.3.6, 6.5 | Runtime model stores linked `invokedynamic` call sites by defining class and bytecode offset, preserves constant-pool index, bootstrap method index, name, and descriptor, and rejects rebinding an already linked call site | `jvm-runtime` | `JvmInvokeDynamicCallSiteRegistryTest` | IMPLEMENTED |
