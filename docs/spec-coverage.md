...
| Object and arrays | `new`, `newarray`, `anewarray`, `arraylength`; `multianewarray` now rejects zero dimensions, while positive-dimension recursive allocation remains TBD | `jvm-interpreter`, `jvm-runtime`, `jvm-verifier` | `JvmInterpreterTest.multianewarray rejects zero dimensions` | PENDING |
