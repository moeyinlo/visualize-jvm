...
| Interpreter missing opcodes | The opcode gate still classifies selected opcodes as not yet implemented, especially `invokeinterface` and `invokedynamic`; `invokeinterface` now has runtime `count` and fourth-zero operand validation plus receiver-class concrete interpreted/native implementation dispatch, but still lacks default/maximally-specific interface selection; `multianewarray` now has focused zero-dimension, negative-count, one-dimensional, and recursive multi-dimensional allocation coverage, and monitor instructions now have focused interpreter execution slices, null-object exception coverage, and executable opcode coverage, but contended monitor blocking and broader synchronization behavior remain pending. | Convert each `OpcodeExecutionCoverage.NotYetImplemented` entry into tested execution or precise spec error behavior. |
