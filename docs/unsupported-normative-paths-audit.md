...
| Dynamic constants and invokedynamic | Classfile structures are parsed, runtime `invokedynamic` call-site specs are resolved from `CONSTANT_InvokeDynamic_info`, linked call sites have a cache model, `BootstrapMethods` entries can be mapped and selected by zero-based bootstrap index, bootstrap invocation inputs now materialize method handles plus static argument records in both runtime resolver and interpreter pre-link execution, and interpreter `invokedynamic` now materializes bootstrap method guest arguments for all already-resolved loadable static argument kinds before the pending bootstrap-execution boundary. Bootstrap method execution, non-static MethodHandle target binding, dynamic constant evaluation/caching, and executable `invokedynamic` target dispatch remain pending; bootstrap-return validation now requires guest `CallSite` objects, extracts their target `MethodHandle` references, resolves `InvokeStatic` targets through the class hierarchy, and caches those targets in the linked call-site registry. | Implement method-handle bootstrap invocation, method types, call-site linkage, dynamic constant caching, target dispatch, and GUI events. |
