...
| Dynamic constants and invokedynamic | Classfile structures are parsed, runtime `invokedynamic` call-site specs are now resolved from `CONSTANT_InvokeDynamic_info`, and linked call sites have a cache model, but bootstrap method invocation, call-site target binding, dynamic constants, and executable `invokedynamic` target dispatch remain pending. | Implement bootstrap method resolution, method handles/types, call-site linkage, dynamic constant caching, target dispatch, and GUI events. |
