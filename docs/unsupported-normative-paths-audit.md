...
| Dynamic constants and invokedynamic | Classfile structures are parsed, runtime `invokedynamic` call-site specs are resolved from `CONSTANT_InvokeDynamic_info`, linked call sites have a cache model, `BootstrapMethods` entries can be mapped and selected by zero-based bootstrap index, bootstrap invocation inputs now materialize method handles plus static argument records in both runtime resolver and interpreter pre-link execution, and interpreter `invokedynamic` now materializes bootstrap method guest arguments for all already-resolved loadable static argument kinds before the pending bootstrap-execution boundary. Bootstrap method execution, bootstrap-return validation, call-site target binding, dynamic constant evaluation/caching, and executable `invokedynamic` target dispatch remain pending; the heap now has guest `CallSite` objects that retain target `MethodHandle` references for that binding step. | Implement method-handle bootstrap invocation, method types, call-site linkage, dynamic constant caching, target dispatch, and GUI events. |
