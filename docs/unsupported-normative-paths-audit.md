...
| Interpreter missing opcodes | The opcode gate still classifies selected opcodes as not yet implemented, especially `invokeinterface`, `invokedynamic`, and some object/control-flow behaviors; monitor instructions now have basic focused interpreter execution slices, but unowned monitor-exit guest exception conversion remains pending. | Convert each `OpcodeExecutionCoverage.NotYetImplemented` entry into tested execution or precise spec error behavior. |
