...
| Dynamic constants and invokedynamic | Classfile structures are parsed, runtime `invokedynamic` call-site specs are resolved from `CONSTANT_InvokeDynamic_info`, linked call sites have a cache model, `BootstrapMethods` entries can be mapped and selected by zero-based bootstrap index, bootstrap invocation inputs now materialize method handles plus static argument records in both runtime resolver and interpreter pre-link execution, and interpreter `invokedynamic` now materializes bootstrap method guest arguments for all already-resolved loadable static argument kinds before the pending bootstrap-execution boundary. Interpreter execution now checks the linked call-site cache by defining class and bytecode offset, can dispatch cached static targets with descriptor arguments through the shared static invocation path, rejects mismatched cached target descriptors, executes `InvokeStatic` bootstrap method handles through the shared static invocation path, validates guest `CallSite` bootstrap results, binds their returned target `MethodHandle` while preserving its reference kind, dispatches `InvokeStatic` targets, and keeps non-static target MethodHandle execution as an explicit pending dispatch path; non-static bootstrap MethodHandle execution, non-static target MethodHandle dispatch, dynamic constant evaluation/caching, and broader bootstrap adaptation semantics remain pending; bootstrap-return validation now requires guest `CallSite` objects, extracts their target `MethodHandle` references, resolves `InvokeStatic` targets through the class hierarchy, and caches those targets in the linked call-site registry. | Implement method-handle bootstrap invocation, method types, call-site linkage, dynamic constant caching, remaining target dispatch, and GUI events. |
