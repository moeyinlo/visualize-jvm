...
| Dynamic constants and invokedynamic | Classfile structures are parsed, runtime `invokedynamic` call-site specs are resolved from `CONSTANT_InvokeDynamic_info`, linked call sites have a cache model, `BootstrapMethods` entries can be mapped and selected by zero-based bootstrap index, bootstrap invocation inputs now materialize method handles plus static argument records in both runtime resolver and interpreter pre-link execution, and interpreter `invokedynamic` now materializes bootstrap method guest arguments for all already-resolved loadable static argument kinds before bootstrap execution. Interpreter execution now checks the linked call-site cache by defining class and bytecode offset, can dispatch cached static targets with descriptor arguments through the shared static invocation path, cached virtual targets through explicit receiver dispatch, cached special targets through direct explicit-receiver dispatch, and cached interface targets through receiver-class interface method selection, rejects mismatched cached target descriptors, executes `InvokeStatic` bootstrap method handles through the shared static invocation path, validates guest `CallSite` bootstrap results, binds their returned target `MethodHandle` while preserving its reference kind, dispatches `InvokeStatic`, `InvokeVirtual`, non-constructor `InvokeSpecial`, and `InvokeInterface` targets, and keeps constructor/special and broader adaptation execution as explicit pending dispatch paths; runtime bootstrap-return validation now requires guest `CallSite` objects, extracts their target `MethodHandle` references, resolves `InvokeStatic`, `InvokeVirtual`, `InvokeSpecial`, `InvokeInterface`, and constructor `NewInvokeSpecial` targets through the class hierarchy, rejects static methods for virtual/special/interface/constructor MethodHandle targets, rejects `NewInvokeSpecial` targets that are not `<init>`, and caches those targets in the linked call-site registry. Non-static bootstrap MethodHandle execution, constructor target execution, dynamic constant evaluation/caching, and broader bootstrap adaptation semantics remain pending. | Implement method-handle bootstrap invocation, method types, call-site linkage, dynamic constant caching, remaining target dispatch, and GUI events. |
