...
| Dynamic constants and invokedynamic | Classfile structures are parsed, runtime `invokedynamic` call-site specs are resolved from `CONSTANT_InvokeDynamic_info`, linked call sites have a cache model, and `BootstrapMethods` entries can be mapped and selected by zero-based bootstrap index, but bootstrap method invocation, call-site target binding, dynamic constants, and executable `invokedynamic` target dispatch remain pending. | Implement method-handle bootstrap invocation, method types, call-site linkage, dynamic constant caching, target dispatch, and GUI events. |
