...
| Interpreter missing opcodes | The opcode gate still classifies selected opcodes as not yet implemented, especially `invokeinterface`, `invokedynamic`, and positive-dimension `multianewarray` allocation; `multianewarray` zero dimensions and negative counts are covered, and monitor instructions now have focused interpreter execution slices, null-object exception coverage, and executable opcode coverage, but contended monitor blocking and broader synchronization behavior remain pending. | Convert each `OpcodeExecutionCoverage.NotYetImplemented` entry into tested execution or precise spec error behavior. |
