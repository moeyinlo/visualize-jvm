...
class JvmClassPathJarLoaderTest {
    @Test
    fun `loads a class from a jar classpath entry by internal name`() {
        val jarPath = Files.createTempDirectory("visualize-jvm-classpath-jar-test").resolve("classes.jar")
        writeJarClass(jarPath, "pkg/Example", ClassFileWriter.writeClassFile(classFile("pkg/Example")))
        val methodArea = JvmMethodArea()
        val loader = JvmClassPathJarLoader(jarPath, methodArea)
