...
object InitializationCoverage {
    val entries: List<InitializationCoverageEntry> = listOf(
        ...
        InitializationCoverageEntry(
            ...
            specSection = "JVMS 5.5 Initialization",
            currentComponent = "new, getstatic, putstatic, invokestatic, and cached invokedynamic InvokeStatic targets mark prepared target classes initialized when no class initializer is present; real <clinit> execution, recursive ordering, waiting, and error transitions remain pending",
            status = InitializationCoverageStatus.PartiallyImplemented,
            coveringTestClass = "JvmInterpreterTest",
        ),
        InitializationCoverageEntry(
