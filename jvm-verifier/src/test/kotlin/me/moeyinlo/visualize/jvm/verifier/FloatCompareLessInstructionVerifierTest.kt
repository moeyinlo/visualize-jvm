...
class FloatCompareLessInstructionVerifierTest {
    @Test
    fun `fcmpl replaces two floats on top with int`() {
        val frame = frame(stack = listOf(VerificationType.Long, VerificationType.Float, VerificationType.Float))

        val nextFrame = FloatCompareLessInstructionVerifier.verify(frame = frame, maxStack = 4)
    ...
    @Test
    fun `fcmpl rejects a non float top operand`() {
        val exception = assertFailsWith<MethodVerificationException> {
            FloatCompareLessInstructionVerifier.verify(
    ...
    @Test
    fun `fcmpl rejects a non float next operand`() {
        val exception = assertFailsWith<MethodVerificationException> {
            FloatCompareLessInstructionVerifier.verify(
    ...
    @Test
    fun `fcmpl rejects an empty operand stack`() {
        val exception = assertFailsWith<MethodVerificationException> {
            FloatCompareLessInstructionVerifier.verify(frame = frame(stack = emptyList()), maxStack = 1)
        }
    ...
    @Test
    fun `fcmpl rejects an incoming stack exceeding max stack`() {
        val exception = assertFailsWith<MethodVerificationException> {
            FloatCompareLessInstructionVerifier.verify(
    ...
    private fun frame(stack: List<VerificationType>): VerificationFrameState =
        VerificationFrameState(bytecodeOffset = 149, locals = emptyList(), stack = stack)
}
