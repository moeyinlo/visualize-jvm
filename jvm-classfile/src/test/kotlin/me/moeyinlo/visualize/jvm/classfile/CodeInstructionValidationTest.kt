...
class CodeInstructionValidationTest {
        ...
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                ...
                constantPool = ConstantPool.fromEntries(
                    listOf(
                        ConstantUtf8Entry("Code", byteArrayOf()),
                        ConstantMethodHandleEntry(MethodHandleReferenceKind.InvokeVirtual, ConstantPoolIndex(3)),
    ...
    @Test
    fun `rejects ldc method handle interface references whose target is not an interface method reference`() {
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                code = byteArrayOf(0x12, 2, 0xB1.toByte()),
                constantPool = ConstantPool.fromEntries(
    ...
    @Test
    fun `rejects ldc method handle new invokespecial references whose target is not a method reference`() {
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                code = byteArrayOf(0x12, 2, 0xB1.toByte()),
                constantPool = ConstantPool.fromEntries(
    ...
    @Test
    fun `rejects ldc method handle interface references whose interface methodref class index does not point to class`() {
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                code = byteArrayOf(0x12, 2, 0xB1.toByte()),
                constantPool = ConstantPool.fromEntries(
                    listOf(
                        ConstantUtf8Entry("Code", byteArrayOf()),
                        ConstantMethodHandleEntry(MethodHandleReferenceKind.InvokeInterface, ConstantPoolIndex(3)),
                        ConstantInterfaceMethodRefEntry(ConstantPoolIndex(5), ConstantPoolIndex(4)),
    ...
    @Test
    fun `rejects ldc method handle new invokespecial references whose descriptor is not a method descriptor`() {
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                code = byteArrayOf(0x12, 2, 0xB1.toByte()),
                constantPool = ConstantPool.fromEntries(
    ...
    @Test
    fun `rejects ldc method handle new invokespecial references whose methodref class index does not point to class`() {
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                code = byteArrayOf(0x12, 2, 0xB1.toByte()),
                constantPool = ConstantPool.fromEntries(
                    listOf(
                        ConstantUtf8Entry("Code", byteArrayOf()),
                        ConstantMethodHandleEntry(MethodHandleReferenceKind.NewInvokeSpecial, ConstantPoolIndex(3)),
                        ConstantMethodRefEntry(ConstantPoolIndex(5), ConstantPoolIndex(4)),
                        ConstantNameAndTypeEntry(ConstantPoolIndex(6), ConstantPoolIndex(7)),
                        ConstantUtf8Entry("Example", byteArrayOf()),
                        ConstantUtf8Entry("<init>", byteArrayOf()),
                        ConstantUtf8Entry("()V", byteArrayOf()),
    ...
    fun `rejects ldc method handle new invokespecial references whose target is not init`() {
        ...
        assertTrue(failure.message.orEmpty().contains("run"), failure.message)
    }

    @Test
    fun `rejects ldc method handle invokevirtual references whose target is a special method name`() {
    ...
    @Test
    fun `rejects ldc method handle invokevirtual references whose descriptor is not a method descriptor`() {
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                code = byteArrayOf(0x12, 2, 0xB1.toByte()),
                constantPool = ConstantPool.fromEntries(
    ...
    @Test
    fun `rejects ldc method handle invokevirtual references whose methodref class index does not point to class`() {
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                code = byteArrayOf(0x12, 2, 0xB1.toByte()),
                constantPool = ConstantPool.fromEntries(
    ...
    @Test
    fun `rejects ldc method handle static or special references whose descriptor is not a method descriptor`() {
        val cases = listOf(MethodHandleReferenceKind.InvokeStatic, MethodHandleReferenceKind.InvokeSpecial)
        cases.forEach { referenceKind ->
            val failure = assertFailsWith<ClassFileFormatException> {
                parseCodeAttribute(
                    code = byteArrayOf(0x12, 2, 0xB1.toByte()),
                    constantPool = ConstantPool.fromEntries(
    ...
    @Test
    fun `rejects ldc method handle static or special references whose methodref class index does not point to class`() {
        val cases = listOf(MethodHandleReferenceKind.InvokeStatic, MethodHandleReferenceKind.InvokeSpecial)
        cases.forEach { referenceKind ->
            val failure = assertFailsWith<ClassFileFormatException> {
                parseCodeAttribute(
    ...
    @Test
    fun `rejects ldc method handle interface references whose descriptor is not a method descriptor`() {
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                code = byteArrayOf(0x12, 2, 0xB1.toByte()),
                constantPool = ConstantPool.fromEntries(
    ...
    @Test
    fun `rejects ldc method type constants whose descriptor is not a method descriptor`() {
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                code = byteArrayOf(0x12, 2, 0xB1.toByte()),
                constantPool = ConstantPool.fromEntries(
                    listOf(
                        ConstantUtf8Entry("Code", byteArrayOf()),
                        ConstantMethodTypeEntry(ConstantPoolIndex(3)),
                        ConstantUtf8Entry("I", byteArrayOf()),
                    ),
                ),
            )
    ...
    @Test
    fun `rejects invokevirtual operand whose methodref class index does not point to class`() {
        val failure = assertFailsWith<ClassFileFormatException> {
            parseCodeAttribute(
                code = byteArrayOf(0xB6.toByte(), 0, 2, 0xB1.toByte()),
                constantPool = ConstantPool.fromEntries(
