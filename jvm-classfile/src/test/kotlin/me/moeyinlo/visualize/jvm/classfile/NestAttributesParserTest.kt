...
class NestAttributesParserTest {
    ...
    @Test
    fun `rejects NestHost attributes before Java 11`() {
        val constantPool = ConstantPool.fromEntries(
            listOf(
                ConstantUtf8Entry("NestHost", byteArrayOf()),
                ConstantUtf8Entry("pkg/Host", byteArrayOf()),
    ...
    @Test
    fun `rejects NestHost indexes that name array classes`() {
        val constantPool = ConstantPool.fromEntries(
            listOf(
                ConstantUtf8Entry("NestHost", byteArrayOf()),
                ConstantUtf8Entry("[Lpkg/Host;", byteArrayOf()),
                ConstantClassEntry(ConstantPoolIndex(2)),
            ),
        )
        ...
        assertTrue(failure.message.orEmpty().contains("ClassFile.attributes[0].host_class_index"), failure.message)
    ...
    @Test
    fun `rejects NestMembers entries that name array classes`() {
        val constantPool = ConstantPool.fromEntries(
            listOf(
                ConstantUtf8Entry("NestMembers", byteArrayOf()),
                ConstantUtf8Entry("[Lpkg/Member;", byteArrayOf()),
                ConstantClassEntry(ConstantPoolIndex(2)),
            ),
        )
